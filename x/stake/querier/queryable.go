...
const (
	...
	QueryCrossStakeInfoByAscAddress    = "crossStakeInfoByAscAddress"
)
...
func NewQuerier(k keep.Keeper, cdc *codec.Codec) sdk.Querier {
	return func(ctx sdk.Context, path []string, req abci.RequestQuery) (res []byte, err sdk.Error) {
		...
		case QueryCrossStakeInfoByAscAddress:
			p := new(QueryCrossStakeInfoParams)
			ctx, err = RequestPrepare(ctx, k, req, p)
			if err != nil {
				return res, err
			}
			return queryCrossStakeInfoByAscAddress(ctx, cdc, p, k)
		default:
...
func queryCrossStakeInfoByAscAddress(ctx sdk.Context, cdc *codec.Codec, params *QueryCrossStakeInfoParams, k keep.Keeper) ([]byte, sdk.Error) {
	if params.AscAddress.IsEmpty() {
		return []byte{}, sdk.ErrInternal("invalid side chain address")
	}
	delCAoB := types.GetStakeCAoB(params.AscAddress[:], types.DelegateCAoBSalt)
