...
type Params struct {
	...
	FeeFromAscToFcRatio      types.Dec `json:"fee_from_asc_to_fc_ratio"`    // the fee from axc to bc ratio
}
...
func (p *Params) UpdateCheck() error {
	...
	if p.FeeFromAscToFcRatio.LT(types.ZeroDec()) {
		return fmt.Errorf("the fee_from_asc_to_fc_ratio should be no less than 0")
	}
